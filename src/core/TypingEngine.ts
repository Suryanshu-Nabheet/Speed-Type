import { TestMetrics, Keystroke, ReplayData } from './types';
...
export class TypingEngine {
  ...
  private calculateConsistency(): number {
    if (this.keystrokes.length < 10) return 100;
    ...
    for (let i = 0; i < this.keystrokes.length; i += 10) {
      ...
      if (intervalKeystrokes.length === 0) continue;
  ...
  getCharacterAtPosition(position: number): { char: string; status: 'untyped' | 'correct' | 'incorrect' | 'cursor' } {
