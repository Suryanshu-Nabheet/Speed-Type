...
export class LeaderboardManager {
  ...
  private async getLocalLeaderboard(options: LeaderboardOptions): Promise<LeaderboardEntry[]> {
    let whereClause = '';
    const params: any[] = [];
  ...
  async getPersonalBest(profileId: string, mode?: string): Promise<LeaderboardEntry | null> {
  ...
  async getRanking(profileId: string, mode?: string): Promise<{
    rank: number;
    total: number;
    percentile: number;
  }> {
    ...
    const betterResult = this.db.prepare(betterWpmQuery).get(...params) as { count: number };
