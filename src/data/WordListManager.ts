...
export class WordListManager {
  ...
  private ensureWordListsDir(): void {
  ...
  getWordList(name: string): WordList | null {
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        const wordList = JSON.parse(content) as WordList;
  ...
  generateText(
    ...
    const wordLists = this.getAllWordLists().filter(
      wl => wl.category === category && wl.difficulty === difficulty
    );
  ...
  private generateWordsText(wordList: WordList, wordCount: number): string {
  ...
  private generateSentenceText(wordList: WordList, targetWordCount: number): string {
  ...
  private getDefaultWordLists(): WordList[] {
    return [
      {
