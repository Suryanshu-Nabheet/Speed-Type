...
export class RaceServer {
  ...
  private wordListManager = new WordListManager();
  ...
  private handleCreateRace(connectionId: string, messageId: string, data: any): void {
    try {
  ...
  private handleJoinRace(connectionId: string, messageId: string, data: any): void {
  ...
  private handleListRaces(connectionId: string, messageId: string, data: any): void {
    const availableRaces = Array.from(this.races.values()).filter(
      race => race.status === 'waiting' || race.status === 'starting'
    );
    this.sendResponse(connectionId, messageId, true, availableRaces);
  }

  private handleUpdateProgress(connectionId: string, messageId: string, data: any): void {
    const race = this.races.get(data.raceId);
    if (!race) {
      this.sendResponse(connectionId, messageId, false, 'Race not found');
      return;
  ...
  private handleFinishRace(connectionId: string, messageId: string, data: any): void {
    const race = this.races.get(data.raceId);
    if (!race) {
      this.sendResponse(connectionId, messageId, false, 'Race not found');
      return;
  ...
  private sendResponse(connectionId: string, messageId: string, success: boolean, data?: any): void {
  ...
  private broadcastToRace(raceId: string, type: string, data: any): void {
